...
class Solution:
    def getSkyline(self, buildings: List[List[int]]) -> List[List[int]]:
        ...
        for left, right, height in buildings:
            events.append((left, 'Entry', height))
            events.append((right, 'Exit', height))
        events.sort()
        ...
        for i, (x_axis, type, height) in enumerate(events):
            if type == 'Entry':
                building_heights.append(height)
            if type == 'Exit':
                building_heights.remove(height)

            if len(building_heights) == 0:
                continue
            # not the last event, and the next event has different x_axis
            if i < len(events)-1 and events[i + 1][0] == events[i][0]:
                continue
            if max(building_heights) != max_height:
                rslt.append([x_axis, max(building_heights)])
...
sol = Solution()
sol.getSkyline(buildings=buildings)
