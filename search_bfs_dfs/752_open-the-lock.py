...
# '7', '8', '9'. The wheels can rotate freely and wrap around: for example we can turn '9' to be '0',
...
# try to solve it using BFS
class Solution:
    deltas = list(product([0, 1, 2, 3], [True, False]))
    ...
    def openLock(self, deadends: List[str], target: str) -> int:
        ...
        while q:
            for _ in range(len(q)):
                ...
                if curr_nums == target:
                    return distance
                for position, perform_add in Solution.deltas:
                    next_nums = self.dial_the_lock(curr_nums, position, perform_add)
                    if (next_nums not in deadends) and (next_nums not in visited):
                        q.append(next_nums)
    ...
    # position could be 0-3
    # if add, 0+1 = 2,    9+1 = 0
    def dial_the_lock(self, curr_loc_nums, position, add=True):
        l = list(curr_loc_nums)
        num = int(l[position])
        if add:
            l[position] = str((num + 1) % 10)
        else:
            l[position] = str((num - 1) % 10)
